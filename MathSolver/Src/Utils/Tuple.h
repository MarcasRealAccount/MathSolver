...
namespace Utils
{
	...
	public:
		constexpr Tuple(T&& value, Ts&&... values)
	...
	namespace Details
	{
		...
		constexpr void add_unique_ptr(std::vector<std::unique_ptr<T>>& ptrs, const Tuple<Ts...>& tuple)
		{
			using U = Tuple<Ts...>;
		...
		template <class T, DerivedFrom<T>... Ts>
		constexpr void add_unique_ptr(std::vector<std::unique_ptr<T>>& ptrs, Tuple<Ts...>&& tuple)
		{
			using U = Tuple<Ts...>;
		...
		template <class T, DerivedFrom<T>... Ts>
		constexpr std::vector<std::unique_ptr<T>> make_unique_ptrs(const Tuple<Ts...>& tuple)
		{
		...
		template <class T, DerivedFrom<T>... Ts>
		constexpr std::vector<std::unique_ptr<T>> make_unique_ptrs(Tuple<Ts...>&& tuple)
		{
			std::vector<std::unique_ptr<T>> ptrs;
			ptrs.reserve(sizeof...(Ts));
